  ...
  return (
    <div>
      <SectionHead
        ...
        sub="Tener la plantilla del proveedor nos ayuda a extraer mejor los datos. Si no aparece, elige 'Otro proveedor'."
...
const UploadStep2 = () => {
  ...
  function addFiles(files) {
    ...
    // Simulate upload + processing pipeline for each file
    newOnes.forEach((file, idx) => {
      simulateFile(file.id, idx, dispatch);
    });
  }
  ...
  const goNext = () => {
    // Build preview rows from the queue's ready files
    const provider = state.selectedProvider;
    const rows = [];
    state.uploadQueue.forEach((f, fi) => {
      if (f.status !== "ready") return;
      // Generate fake extracted rows tailored to the provider's type
      const ptype = provider.type === "any" ? "electricidad" : provider.type;
      const rowCount = f.extractedCount || (provider.id === "iconstruye-pet" ? 3 : 1);
      for (let i = 0; i < rowCount; i++) {
        // 1 in 4 marked as 'warn' (atypical), 1 in 8 as 'error' (missing field)
        const r = (fi + i) % 8;
        const status = r === 0 ? "error" : r === 1 || r === 2 ? "warn" : "ok";
        const sub = ptype === "combustible" ? ["diesel","kerosene","glp","gas-natural"][i % 4]
                   : ptype === "agua" ? ["potable","gris","riego"][i % 3]
                   : null;
        rows.push({
          id: nextId(),
          date: "2026-03-28",
          sucursal: ["Planta Norte","Planta Sur","Bodega RM","CD Quilicura"][(fi + i) % 4],
          type: ptype,
          subcat: sub,
          provider: provider.name === "Otro proveedor" ? "" : provider.name,
          cantidad: status === "error" ? "" : Math.round((ptype === "electricidad" ? 12480 : ptype === "combustible" ? 3420 : 142) * (1 + (i + fi) * 0.08)),
          costo: status === "error" ? "" : Math.round((ptype === "electricidad" ? 980412 : ptype === "combustible" ? 1847230 : 189420) * (1 + (i + fi) * 0.08)),
          status,
          sourceFile: f.name,
        });
      }
...
// Simulated file pipeline — runs outside reducer using setTimeout
function simulateFile(id, idx, dispatch) {
  // Upload phase: 0 → 100 over ~1200ms
  let prog = 0;
  const interval = setInterval(() => {
    ...
    if (prog >= 100) {
      ...
      // Parsing phase: 1.6 - 2.6s
      setTimeout(() => {
        // Random outcome
        const seed = idx + Math.floor(Math.random() * 7);
        if (seed % 9 === 0) {
          dispatch({ type: "UPLOAD/UPDATE_FILE", id, patch: { status: "error", error: "No pudimos extraer la tabla" } });
        } else {
          const count = (seed % 3) + 1;
          dispatch({ type: "UPLOAD/UPDATE_FILE", id, patch: { status: "ready", extractedCount: count } });
        }
...
Object.assign(window, { UploadView, UploadStep1, UploadStep2, simulateFile });
